<!-- generated:start edge:shortcut-server→iv-solver -->
# Interaction: Shortcut Server → IV Solver

- **Participants:** `shortcut-server` → `iv-solver`
- **Transport:** `Python function call`
<!-- generated:end edge:shortcut-server→iv-solver -->

## Contract

_TODO (agent-owned): request/response shapes and invariants._

## Auth

_TODO (agent-owned): how this interaction is authenticated._

## Flow

_TODO (agent-owned): the sequence of the interaction._

## Code

_TODO (agent-owned): entry points on both sides._
